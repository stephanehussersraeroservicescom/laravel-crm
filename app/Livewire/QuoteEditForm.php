...
class QuoteEditForm extends Component
{
    ...
    protected function loadQuoteData()
    {
        ...
        // Load quote lines
        $this->quote_lines = [];
        foreach ($this->quote->quoteLines as $index => $line) {
            $this->quote_lines[$index] = [
                'root_code' => $line->root_code,
                'part_number' => $line->part_number,
                'description' => $line->description,
                'quantity' => $line->quantity,
                'unit' => $line->unit,
                'standard_price' => $line->standard_price / 100, // Convert from cents
                'final_price' => $line->final_price / 100, // Convert from cents
            ...
            // Load series if root exists
            if ($line->root_code) {
                $this->root_searches[$index] = $line->root_code;
                // Series are no longer stored separately
                $this->available_series[$index] = collect();
                    
                // Try to determine selected series from part number
                if ($line->series_code) {
                    $this->selected_series[$index] = $line->series_code;
                }
            }
        }
    }
    ...
    public function selectRoot($index, $rootCode)
    {
        ...
        if ($root) {
            ...
            // Load available series for this root
    ...
    private function getRootPricing(ProductClass $root, $partNumber = null)
    {
        ...
        if ($customerIdentifier && $this->quote->airline_id) {
            $contractPrice = ContractPrice::findBestPrice(
                $customerIdentifier, 
                $root->root_code, 
                $this->quote->airline_id
            );
            if ($contractPrice) {
                return [
    ...
    public function updatedQuoteLines($value, $property)
    {
        ...
        if (preg_match('/(\d+)\.part_number/', $property, $matches)) {
            ...
            $customerIdentifier = $this->quote->company_name ?: $this->quote->contact_name;
            if ($customerIdentifier && $this->quote->airline_id) {
                $contractPrice = ContractPrice::active()
                    ->where('part_number', $partNumber)
                    ->where(function ($q) use ($customerIdentifier) {
                        $q->where('customer_identifier', $customerIdentifier)
                          ->orWhere('airline_id', $this->quote->airline_id);
                    })
    ...
    public function save()
    {
        ...
        try {
            ...
            // Delete existing quote lines and create new ones
            $this->quote->quoteLines()->delete();
            ...
            session()->flash('error', 'Error updating quote: ' . $e->getMessage());
        }
    }
    ...
    {
        return view('livewire.quote-edit-form');
    }
}